...
} from '@/app/dashboard/(dashboard)/courses/components/generator/steps';
...
import { useAristocratCourseGeneration } from './context';
...
export const AristocratCourseGenerationForm = ({
	...
	const startCourseGenerationMutation = useMutation({
		...trpc.courses.$post.create.mutationOptions(),
		onSuccess: () => {
			setIsModalOpen(false);
			toast.info('El curso está siendo generado.', {
				description: 'Puedes consultar su estado en cualquier momento.',
			});
		},
	});
	...
	const courseGenerationFormInstance = useForm({
		...
		onSubmit: async (data) => {
			startCourseGenerationMutation.mutateAsync({
				difficulty: data.value.difficulty,
				youtubeVideoId: data.value.youtubeVideoId,
				language: availableLanguagesMap[data.value.language],
			});
		},
	});
